  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50">
      <form
        ...
        className="bg-panel border border-panel-border rounded-xl max-w-lg w-full max-h-[85vh] flex flex-col"
      >
        <div className="p-6 pb-4 overflow-y-auto flex flex-col gap-4">
          <h3 className="text-lg font-bold text-parchment-bright">{t('logFind')}</h3>

          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold uppercase tracking-wider text-muted">{t('itemLabel')}</label>
            <select
              required
              value={itemId}
              onChange={e => { setItemId(e.target.value); setStatValues({}); }}
              className="bg-panel-alt border border-panel-border-light rounded-lg px-3 py-2 text-parchment-bright"
            >
          ...
          <div className="flex gap-4">
            <div className="flex flex-col gap-1 flex-1">
              <label className="text-xs font-semibold uppercase tracking-wider text-muted">{t('act')}</label>
              <select
                value={act}
            ...
            <div className="flex flex-col gap-1 flex-1">
              <label className="text-xs font-semibold uppercase tracking-wider text-muted">{t('area')}</label>
              <select
                value={area}
          ...
          <div className="flex gap-4 items-end">
            <div className="flex flex-col gap-1 flex-1">
              <label className="text-xs font-semibold uppercase tracking-wider text-muted">{t('foundDate')}</label>
              <input
                type="date"
                value={foundAt}
          ...
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold uppercase tracking-wider text-muted">{t('notes')}</label>
            <textarea
              value={notes}
        ...
        <div className="flex gap-2 justify-end px-6 py-4 border-t border-panel-border shrink-0">
          <button type="button" onClick={onCancel} className="px-4 py-2 text-sm text-muted hover:text-parchment-bright">
            {t('cancel')}
          </button>
          <button
            type="submit"
            disabled={!selected || saving}
            className="px-4 py-2 rounded-lg bg-gold text-ink-950 font-semibold text-sm hover:bg-gold-bright disabled:opacity-30 transition-colors"
          >
            {saving ? t('saving') : t('saveFind')}
          </button>
        </div>
      </form>
    </div>
