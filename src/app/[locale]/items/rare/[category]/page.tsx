...
export function generateStaticParams() {
  return routing.locales.flatMap(locale =>
    getAffixCategories('rare').map(category => ({ locale, category }))
  );
}

export default async function RareCategoryPage({
  params,
}: {
  ...
  if (!getAffixCategories('rare').includes(category)) notFound();
  ...
  return (
    <main className="flex flex-col items-center py-10 px-4 gap-6 flex-1 w-full">
      <div className="text-center">
        <h1 className="text-3xl font-bold tracking-tight text-parchment-bright">{t('rarePageTitle')}</h1>
        <p className="mt-2 text-sm text-muted max-w-md">{t('rarePageSubtitle')}</p>
      </div>
      <div className="w-full max-w-4xl flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-semibold text-parchment-bright">{categoryLabel}</h2>
          <Link
            href={`/${locale}/items/rare`}
            className="text-sm text-muted hover:text-gold-bright transition-colors"
          >
            {t('backToCategories')}
          </Link>
        </div>
